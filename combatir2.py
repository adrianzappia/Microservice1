...
while True:
    ...
    template_path = 'btn_cruz2.png'
    detected_image,encontre,x,y = detect_object(window, template_path,0.7)
    cv2.imshow('Detected', detected_image)
    print(f'{encontre}')
    if not(encontre):
        template_path = 'btn_compartir.png'
        detected_image,encontre,x,y = detect_object(window, template_path,0.95)
        cv2.imshow('Detected', detected_image)
        if encontre:
            print("presiono")
    ...
    #    time.sleep(1)
    #    cont=0
    #template_path = 'btn_atacar_disable.png'
